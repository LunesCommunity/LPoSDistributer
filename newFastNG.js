...
/*                "attachment": "", */
        ...
        relatorio.push({
             "address": address,
             "amountLunes": Number(Math.round(payments[address])),
             "amountZen": Number(Math.round(zen[address] * Math.pow(10, 2)))
        })



    }
    fs.writeFile(config.relatorio, JSON.stringify(relatorio), {}, function(err) {
        if (!err) {
            console.log('payments written to ' + config.relatorio + '!');
        } else {
            console.log(err);
        }
    });
